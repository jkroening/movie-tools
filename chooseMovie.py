...
## your Netflix queue in a file called queue_body.html. the block of interest
...
def parseTMDB(r):
    ...
    year =  int(mov['release_date'].split("-")[0])
    genres = mov['genres']
    imdb_id = str(mov['imdb_id'])
    overview = unidecode(mov['overview'])
    tagline = unidecode(mov['tagline'])
...
print("To update streaming info, type 'update' as a command line argument.\n")
update_streams = False
if len(sys.argv) > 1:
    update_streams = sys.argv[1]
    if update_streams == "update":
        update_streams = True
        ...
        if cond1 or cond2:
            ...
            if mov.find('div', {'class' : 'maskRated'}) is not None:
                ...
                if update_streams and cisi_id is not None:
                    prev_streams = movies_db.lc[idx, 'streams']
